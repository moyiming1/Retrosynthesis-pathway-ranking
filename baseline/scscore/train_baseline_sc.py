...
from baseline.scscore import train, SCNN, loss_marginranking, valid, loss_softmax

from utils.save_load import save_checkpoint
from utils.scheduler import build_lr_scheduler
from features import get_sc_dataloader
...
scheduler_args = {'init_lr': 0.01, 'final_lr': 0.0005}

# Data loader
# add dataloader
valid_loader = get_sc_dataloader(data_path=data_path, job_type='valid',
                                 batch_size=batch_size, num_workers=num_workers, device=device)
test_loader = get_sc_dataloader(data_path=data_path, job_type='test',
                                batch_size=batch_size, num_workers=num_workers, device=device)
...
# build model
model = SCNN(13, 1, linear_pool=linear_pool, layers=layers).to(device)
...
writer = SummaryWriter(os.path.join(save_path, 'tensorboard/train_03292020_min_layer5' + label))
...
for epoch in range(num_epochs):
    train_loader = get_sc_dataloader(data_path=data_path, job_type='train',
                                     batch_size=batch_size, num_workers=num_workers, device=device)
