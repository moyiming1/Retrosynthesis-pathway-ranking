...
class SCNN(torch.nn.Module):
    ...
    def __init__(self, in_features, out_features, hidden_size=2048, layers=3, dropout=0.0, linear_pool='mean'):
    ...
    def forward(self, features, num_linear_paths):
        scores = self.nn(features)

        score_split = torch.split(scores, num_linear_paths, dim=0)

        if self.linear_pool == 'mean':
            score_tree = torch.stack([torch.mean(x) for x in score_split])
        elif self.linear_pool == 'min':
            score_tree = torch.stack([torch.min(x) for x in score_split])
