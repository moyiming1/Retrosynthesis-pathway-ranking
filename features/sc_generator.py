...
from data_prep.utils import convert_one_record_to_sc, merge_into_batch
            ...
            with open(file, 'rb') as fh:
                while True:
                    try:
                        ...
                        batch_inputs.append(convert_one_record_to_sc(one_record, max_step=self.max_step))
...
def get_sc_dataloader(data_path, job_type='train', max_step=12,  device=torch.device('cpu'),
                      batch_size=4, num_workers=0, shuffle=False):
    sc_dataset = ScDataset(data_path, job_type, batch_size=batch_size,
...
if __name__ == '__main__':
    data_path = '/data/yanfei/Projects/Pathway_ranking/data/all_training_data/all_training_data_with_sc'
    data_loader = get_sc_dataloader(data_path, job_type='train', num_workers=4)
