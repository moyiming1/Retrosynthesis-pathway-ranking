...
class SCScorer():
    ...
    def restore(self, weight_path='utils/SCscore/models/full_reaxys_model_2048bool/model.ckpt-10654.as_numpy.pickle', FP_rad=FP_rad, FP_len=FP_len):
    ...
    def get_score_from_fp_v1(self, fp='', depth=12, fp_len=2048):
        '''
        generate scscore from fingerpring
        :param fp_len:
        :param depth:
        :param fp: a sparse matrix with shape [depth*fp_len,]
        :return:
        '''
        paths_fp = np.reshape(np.array(fp.todense()), [-1, depth, fp_len])

        paths_scores = []
        for path_fp in tqdm(paths_fp):
            path_scores = []
            for step_fp in path_fp:
                if sum(step_fp) == 0:
                    path_scores.append(0.)
                    continue
                else:
                    cur_score = self.apply(step_fp)
                    path_scores.append(cur_score)
            paths_scores.append(path_scores)
        return paths_scores

    def _load_vars(self, weight_path):
        if weight_path.endswith('pickle'):
            import pandas as pd
            self.vars = pd.read_pickle(weight_path)
...
if __name__ == '__main__':
    ...
    model = SCScorer()
    model.restore(
        os.path.join(project_root, 'models', 'full_reaxys_model_2048bool', 'model.ckpt-10654.as_numpy.json.gz'),
        FP_len=2048)
    rxnsmi = 'CCc1ccc(C(C)(C)c2[nH]c3cc(C#N)ccc3c2C(=O)OCOCCOC)cc1N>>CCc1ccc(C(C)(C)c2[nH]c3cc(C#N)ccc3c2C(=O)OCOCCOC)cc1Br'
    ...
    for i in range(10000):
        if i % 100 == 0:
            ...
            print(model.get_score_from_smi(psmi)[1] - model.get_score_from_smi(rsmi)[1])
    # model = SCScorer()
